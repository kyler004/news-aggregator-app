...
const Header: React.FC<HeaderProps> = ({
  ...
  // Keep local input in sync if searchQuery is reset externally
  useEffect(() => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSearchQuery(inputValue.trim()); // Trigger API fetch
  };
      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4 flex-1 max-w-2xl mx-8">
          <form onSubmit={handleSearchSubmit} className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search news and hit Enter..."
              value={inputValue}
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-800 rounded-lg p-1 transition-colors duration-200">
            <span className="text-slate-500 dark:text-slate-400 text-sm mr-2 ml-1">
              VIEW:
            </span>
            <button
